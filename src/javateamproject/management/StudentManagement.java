...
public class StudentManagement {
    ...
    //수강생 전체 목록
    public void inquiryStudent() throws InterruptedException {
        System.out.println("\n수강생 목록을 조회합니다...");
        // 기능 구현
    ...
    public  void searchStudent() throws InterruptedException {
        ...
        try{
            ...
            System.out.println("과목: " +student.getSubjectids());
    ...
    ////담당 수정님
    //상태수정
    public  void setConditionStudent() throws InterruptedException {
    ...
    //수강생 상태별 조회
    public void inquiryConditionStudent() throws InterruptedException {
    ...
    private String getStudentNumFromUser() {
    ...
    private boolean isValidStudentNum(String studentNum) {
    ...
    public Student searchGetStudent() {
        String studentId;
        while (true){

            studentId = getStudentNumFromUser();
            if (store.optionalScoreByStudentId(studentId).isEmpty()) {
                System.out.println("점수 정보가 존재 하지 않는 수강생입니다.");
            }else{
                Optional<Student> result= optionalStudent(studentId);
                if(result.isPresent()) return result.get();
            }
        }
    }
    public  Optional<Student> optionalStudent(String studentId){
        return store.getStudentStore().stream()
                .filter(student -> student.getStudentId().equals(studentId))
                .findFirst();
    }
    public  boolean isExistCondition(ConditionType conditionType){
        Optional<Student> result = store.getStudentStore().stream()
                .filter(student -> student.getCondition()==conditionType)
                .findFirst();
        return result.isPresent();
    }
    private Student searchGetStudentAtStudent(){
        while(true){
