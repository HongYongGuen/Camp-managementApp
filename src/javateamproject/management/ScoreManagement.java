...
public class ScoreManagement {
    ...
    Store store;
    public ScoreManagement(SubjectManagement subjectManagement, StudentManagement studentManagement){
    ...
    public  void addScore() throws InterruptedException {
        ...
        do{
            ...
            if( isFullRound(student.getStudentId(),subjectName)){
                System.out.println("해당 과목은 모든 회차를 입력했습니다.");
            }
...
//            }
            Optional<Score> result = store.optionalScore(student.getStudentId(), subjectName, round);
    ...
    private boolean isFullRound(String studentId, String subjectId){
    ...
    //점수 수정
    public void modScore() throws InterruptedException {
...
//            }
            ...
            // (4) 과목정보와 회차가 동일한 경우 (다시 받을지/예외처리 할지)
            Optional<Score> result = store.optionalScore(student.getStudentId(), subjecId, round);
            if (result.isEmpty()) {
    ...
    public boolean isScoreExistBySubjectId(String subjectId,String studentId) {
    ...
    //수강생 과복별 시험 회차 등급 조회
    public void inqScore() throws InterruptedException {
        //(0) 학생 목록 보여주기
        studentManagement.inquiryStudent();
        ...
        do {
            student = studentManagement.searchGetStudent();
            if (store.optionalScoreByStudentId(student.getStudentId()).isEmpty()) {
                System.out.println("점수 정보가 존재 하지 않는 수강생입니다.");
            }
        }while(store.optionalScoreByStudentId(student.getStudentId()).isEmpty());
    ...
    //여기서부터 경민님
    // (1) 수강생의 과목별 평균 등급 조회
    public void inquiryStudentAverageBySubject() throws InterruptedException {
    ...
    public void inquiryStudentAverageByStatus() {
        ...
        for (Student student : students) {
            ...
            List<String> requiredSubjects = student.getMustSubjectIds();
        ...
        for (String subjectId : totalScores.keySet()) {
            ...
            // 과목명을 가져오기!
            String subjectName = store.getSubjectNameBySubjectId(subjectId);
            String grade = calculateGrade((int)averageScore,store.getSubjectTypeBySubjectId(subjectId));
            // 평균 등급을 출력
...
//7. void inquiryScoreAtStudent()
    public void inquirySubjectGrades(Student student, String subjectName) throws InterruptedException {
    ...
    private String calculateGrade(int score, SubjectType subjectType){
        String grade;
